...
def upgrade() -> None:
    ...
    op.create_table('user_passes',
        ...
        sa.Column('tool_ids', JSONB, server_default=sa.text("'[]'::jsonb"), nullable=False),
        sa.Column('tools_count', sa.Integer(), nullable=False),
        sa.Column('uses_per_day', sa.Integer(), nullable=False),
        sa.Column('uses_today', sa.Integer(), server_default=sa.text('0'), nullable=False),
        sa.Column('uses_reset_date', sa.String(10), nullable=True),
        sa.Column('source', sa.String(20), nullable=False),
        sa.Column('purchased_at', TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_index('ix_user_passes_active_lookup', 'user_passes', ['user_id', 'is_active', 'expires_at'], schema='auth')
    ...
    op.create_index('ix_user_credits_user_id', 'user_credits', ['user_id'], schema='auth')
    op.create_index('ix_user_credits_active_lookup', 'user_credits', ['user_id', 'credits_remaining'], schema='auth')
    op.create_check_constraint('ck_credits_remaining_valid', 'user_credits',
        'credits_remaining >= 0 AND credits_remaining <= credits_total', schema='auth')

    # ── visitor_usage ────────────────────────────────────────────────────────
    op.create_table('visitor_usage',
        sa.Column('id', UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
        sa.Column('fingerprint', sa.String(64), nullable=False),
        sa.Column('ip_address', sa.String(45), nullable=False),
        sa.Column('tool_uses_today', JSONB, server_default=sa.text("'{}'::jsonb"), nullable=False),
        sa.Column('reset_date', sa.String(10), nullable=True),
        sa.Column('created_at', TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
        sa.PrimaryKeyConstraint('id'),
        schema='auth',
    )
    op.create_index('ix_visitor_usage_fingerprint', 'visitor_usage', ['fingerprint'], schema='auth')
    op.create_index('ix_visitor_usage_ip_address', 'visitor_usage', ['ip_address'], schema='auth')

    # ── Update users table ───────────────────────────────────────────────────
    op.add_column('users', sa.Column('tool_uses_today', JSONB, server_default=sa.text("'{}'::jsonb"), nullable=False), schema='auth')
    ...
    op.add_column('users', sa.Column('referral_code', sa.String(20), nullable=True), schema='auth')
    op.add_column('users', sa.Column('referred_by', UUID(as_uuid=True), nullable=True), schema='auth')
    op.add_column('users', sa.Column('region', sa.String(5), nullable=True), schema='auth')
    op.create_unique_constraint('uq_users_referral_code', 'users', ['referral_code'], schema='auth')
    op.create_foreign_key('fk_users_referred_by', 'users', 'users', ['referred_by'], ['id'], source_schema='auth', referent_schema='auth', ondelete='SET NULL')


def downgrade() -> None:
    op.drop_constraint('fk_users_referred_by', 'users', schema='auth', type_='foreignkey')
    ...
    op.drop_constraint('ck_credits_remaining_valid', 'user_credits', schema='auth', type_='check')
