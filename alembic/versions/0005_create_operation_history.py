...
def upgrade() -> None:
    ...
    op.create_index('ix_operation_history_user_id', 'operation_history', ['user_id'], schema='activity')
    op.create_index('ix_operation_history_tool_id', 'operation_history', ['tool_id'], schema='activity')
    op.create_index('ix_operation_history_created_at', 'operation_history', ['created_at'], schema='activity')
    # Composite index for paginated user queries (most common access pattern)
    op.create_index('ix_operation_history_user_created', 'operation_history', ['user_id', 'created_at'], schema='activity')


def downgrade() -> None:
    op.drop_index('ix_operation_history_user_created', 'operation_history', schema='activity')
    op.drop_index('ix_operation_history_created_at', 'operation_history', schema='activity')
    op.drop_index('ix_operation_history_tool_id', 'operation_history', schema='activity')
    op.drop_index('ix_operation_history_user_id', 'operation_history', schema='activity')
